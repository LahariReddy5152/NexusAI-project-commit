/**
 * Priority 3B — Spring Boot, AI, Prompt, OpenAI, RAG, LangChain
 */
...
const TEMPLATE_PATTERNS = [
  /print\("demonstrating .+ with a minimal python workflow"\)/i,
  /spring boot \w+: .+ essential for production microservices/i,
  /integrates with spring's application context and boot's auto-configuration model/i,
  /production spring services rely on \w+ for reliability/i,
  /copilots use \w+ to stay on-brand/i,
  /apply \w+ with eval datasets and guardrails/i,
    ...
    for (const field of REQUIRED_STRING_FIELDS) {
      ...
      for (const pat of TEMPLATE_PATTERNS) {
        if (pat.test(String(val || ""))) {
          issues.push(`${label} "${lesson.title}": template in ${field}`);
        }
      }
    }
